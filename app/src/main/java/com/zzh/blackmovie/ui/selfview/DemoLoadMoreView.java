...
public class DemoLoadMoreView extends BaseLoadMoreView {
    ...
    public void onDrawLoadMore(Canvas c, RecyclerView parent) {
        ...
        //        int cx = (right - left) / 2 ; 不加文字在中间
        int cx = (right - left) / 2 - mCircleOffset;
        c.drawCircle(cx, bottom, mCircleSize, paint);//在中心为（(right - left)/2,bottom）的地方画个半径为mCircleSize的圆，
        paint.setColor(Color.GREEN);// 设置画笔为绿色
        oval.set(cx - mCircleSize, bottom - mCircleSize, cx + mCircleSize, bottom + mCircleSize);// 在Circle小于圈圈大小的地方画圆，这样也就保证了半径为mCircleSize
